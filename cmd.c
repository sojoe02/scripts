...
int main(int argc, char *argv[])
{
	...
	fprintf(stderr,"Sampling rate (1E9/convert_arg)=%f Hz\n",1e9/(float)(cmd->convert_arg));
	fprintf(stderr,"Transfer/File buffer is %d/%d bytes\n", XFRSZ, BUFSZ);
	...
	xfrp = &buf[0];
	while(1){
		ret = read(comedi_fileno(dev),xfrp,XFRSZ);
		if(ret < 0){
			/* some error occurred */
			perror("read");
			break;
		...
		*/
			if( xfr++ < BUFSZ/XFRSZ ) { xfrp += XFRSZ; continue; }
			xfrp = &buf[0];
			xfr = 0;
			write(1, buf, BUFSZ);
		}
	...
	fprintf(stderr,"time: %ld.%06ld,  total: %ld\n", end.tv_sec, end.tv_usec, total);
